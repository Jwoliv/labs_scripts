...
if __name__ == '__main__':
    image_name = "image_1"
    image = read_jpg(f"images/{image_name}.jpg")  # Читаємо зображення з файлу
    message = """
    ...
    write_jpg(f"./images/{image_name}_seq_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.jpg", image_seq)

    key = 340698234968  # Ключ для випадкового вбудовування
    image_rand = embed_random(image, message, key)
    write_jpg(f"./images/{image_name}_random_{datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.jpg", image_rand)
